...
FbxParts::FbxParts() :ppIndexBuffer_(nullptr),pMaterial_(nullptr),
...
HRESULT FbxParts::Init(FbxNode* _pNode)
{
	//ノードからメッシュの情報を取得
	...
	polygonVertexCount_ = mesh->GetPolygonVertexCount();
...
void FbxParts::InitVertex(fbxsdk::FbxMesh* _mesh)
{
	...
	for (DWORD poly = 0; poly < polygonCount_; poly++)
	{
		for (int vertex = 0; vertex < 3; vertex++)
		{
			...
			FbxVector4 Normal;
			_mesh->GetPolygonVertexNormal(poly, vertex, Normal);
	...
	int m_dwNumUV = _mesh->GetTextureUVCount();
	FbxLayerElementUV* pUV = _mesh->GetLayer(0)->GetUVs();
	...
	bd_vertex.ByteWidth = sizeof(VERTEX) * _mesh->GetControlPointsCount();
...
void FbxParts::InitMaterial(fbxsdk::FbxNode* _pNode)
{
	//▼マテリアルバッファの作成
	materialCount_ = _pNode->GetMaterialCount();
...
void FbxParts::InitTexture(fbxsdk::FbxSurfaceMaterial* _pMaterial, const DWORD& _i)
{
	pMaterial_[_i].pTexture = nullptr;
	//▼テクスチャー情報の取得
	...
	if (fileTextureCount > 0)
	{
		...
		pMaterial_[_i].pTexture = new Texture;
		pMaterial_[_i].pTexture->Load(name);
	}

}

void FbxParts::InitIndex(fbxsdk::FbxMesh* _mesh)
{
	...
	for (DWORD i = 0; i < materialCount_; i++)
	{
		...
		//▼ポリゴンを構成する三角形平面が、「頂点バッファ」内のどの頂点を利用しているかを調べる
		for (DWORD j = 0; j < polygonCount_; j++)
		{
			...
			if (mtlId == i)
			{
				for (DWORD k = 0; k < 3; k++)
				{
					pIndex[count + k] = _mesh->GetPolygonVertex(j, k);
				}
				count += 3;
			}
		}
...
void FbxParts::InitSkelton(FbxMesh* _pMesh)
{
...
void FbxParts::Draw(Transform& _transform)
{
	...
	for (DWORD i = 0; i < materialCount_; i++)
	{
		...
		CONSTANT_BUFFER cb;
		cb.worldVewProj = XMMatrixTranspose(_transform.GetWorldMatrix() * pCamera->GetViewMatrix() * pCamera->GetProjectionMatrix());						// リソースへ送る値をセット
		cb.world = XMMatrixTranspose(_transform.GetWorldMatrix());
		cb.normalTrans = XMMatrixTranspose(_transform.matRotate_ * XMMatrixInverse(nullptr, _transform.matScale_));
		...
		for (int i = 0; i <= 1; i++)
		{
			cb.cameraPosition = XMFLOAT4(pCamera->GetPositionFl(i).x, pCamera->GetPositionFl(i).y, pCamera->GetPositionFl(i).z, 0.0f);
...
void FbxParts::DrawSkinAnime(Transform& _transform, FbxTime _time)
{
	//▼ボーンごとの現在の行列を取得する
	for (int i = 0; i < numBone_; i++)
	{
...
void FbxParts::DrawMeshAnime(Transform& _transform, FbxTime _time, FbxScene* _scene)
{
	// その瞬間の自分の姿勢行列を得る
	//FbxAnimEvaluator *evaluator = _scene->GetAnimationEvaluator();
	//FbxMatrix mCurrentOrentation = evaluator->GetNodeGlobalTransform(_pNode, _time);
...
bool FbxParts::GetBonePosition(std::string _boneName, XMFLOAT3* _position)
{
	for (int i = 0; i < numBone_; i++)
	{
...
void FbxParts::RayCast(RayCastData* _data)
{
	_data->hit = FALSE;
	...
	for (DWORD i = 0; i < materialCount_; i++)
	{
		...
		for (DWORD j = 0; j < pMaterial_[i].polygonCount; j++)
		{
			...
			hit = Direct3D::Intersect(_data->start, _data->dir, ver[0], ver[1], ver[2], &dist);


			if (hit && dist < _data->dist)
			{
				_data->hit = TRUE;
				_data->dist = dist;
			}
